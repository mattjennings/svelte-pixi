...
---
{/* 
NOTE: examples currently break because of some svelte update infinite loop, I think related to when AssetsLoader is used 
as a child of another AssetsLoader. Possibly https://github.com/sveltejs/svelte/issues/9652
*/}
...
```svelte livee assets=["/assets/adventurer-spritesheet.json"]
<script>
  ...
  let textures = $state(spritesheet.animations['adventurer-idle'])
