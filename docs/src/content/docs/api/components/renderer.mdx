...
`Renderer` doesn't offer the same `render="demand"` prop as the
[`Application` component does](/docs/components/application#render-on-demand), 
but we can still implement this ourselves: 
