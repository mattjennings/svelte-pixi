...
export function getInterface(source) {
  ...
  return proplist.reduce((list, prop) => {
    let info = {
      type: undefined,
      descr: undefined,
      ...jsdoc.find((v) => v.index === prop.index),
    }
    const propObj = {
      name: prop.name,
...
function extractScriptTag(source) {
  ...
  if (
    match &&
    (match[1].includes(`context="module"`) ||
      match[1].includes(`context='module'`))
  ) {
...
function parseJS(code) {
  ...
  return node.body.reduce((list, node) => {
    if (
      ...
      if (node.declarations)
        node.declarations.forEach(
          (dec) => (vars[dec.id.name] = parseDeclarator(dec, node.kind))
        )
      return list
    }
    ...
    if (node.declaration === null && node.specifiers.length > 0) {
      return [
        ...list,
        ...node.specifiers.reduce((sublist, spec) => {
          sublist.push({
            ...sample,
            ...(vars[spec.local.name] || {}),
    ...
    if (node.type === 'ExportNamedDeclaration' && node.declaration) {
      const decs = node.declaration.declarations
        ? Array.from(node.declaration.declarations)
        : [node.declaration]
      return [
        ...list,
        ...decs.reduce((sublist, dec) => {
